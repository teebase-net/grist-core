################################################################################
## Stage 0: Grab the Official Enterprise Extensions & Dependencies
################################################################################
...
# --- THE FIX: MERGE ENTERPRISE DEPENDENCIES ---
# We copy the enterprise node_modules over ours. This adds 'nodemailer' 
# and other enterprise-only libs without breaking your custom UI build.
COPY --from=enterprise-base /grist/node_modules /grist/node_modules_ent
RUN cp -rn /grist/node_modules_ent/* /grist/node_modules/ && rm -rf /grist/node_modules_ent
...
RUN mkdir -p /grist/static-built && \
    if [ -d "_build/static" ]; then cp -r _build/static/* /grist/static-built/; \
...
################################################################################
## Stage 2 & 3: Python & Sandbox (Unchanged)
################################################################################
...
# 1. Copy the MERGED node_modules and build
...
RUN cp -r /grist/static-built/* /grist/static/ && rm -rf /grist/static-built

RUN useradd -ms /bin/bash grist && chown -R grist:grist /grist /persist
USER grist
WORKDIR /grist
