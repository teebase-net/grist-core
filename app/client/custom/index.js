...
/*===================================================================================
  ...
  Features:
    ...
    - Hides widget chrome (title/filter/config) for widgets using the LabelBlock URL unless user is Owner with unlock_structure.
  ...
  Implementation:
    ...
    - Uses polling and mutation observer for LabelBlock iframe detection
    - Uses the Grist API to get the document name and displays the DEV banner if needed

  Version: v1.5.1
===================================================================================*/

console.log("[Custom Patch] index.js loaded ✅ v1.5.1");
...
function hideLabelBlockControls() {
  ...
  function scheduleLabelBlockCheck() {
    const MAX_ATTEMPTS = 15;
    let attempt = 0;
    const interval = setInterval(() => {
      const count = document.querySelectorAll('iframe[src*="widgets.teebase.net/labelblock"]').length;
      console.log(`[LabelBlock Patch] Poll ${attempt + 1}: found ${count} labelblock iframe(s).`);
      if (count > 0) {
      ...
      if (++attempt >= MAX_ATTEMPTS) {
        clearInterval(interval);
        console.warn("[LabelBlock Patch] ❌ Gave up polling for LabelBlock.");
      }
    }, 500);
  }

  scheduleLabelBlockCheck();

  const labelBlockObserver = new MutationObserver(() => {
    console.log("[LabelBlock Patch] DOM changed, re-checking for labelblock widgets...");
    hideLabelBlockControls();
  });
  labelBlockObserver.observe(document.body, { childList: true, subtree: true });
      ...
      if (data.name && data.name.includes('- DEV')) {
        if (!document.getElementById('custom-global-banner')) {
          ...
          Object.assign(banner.style, {
            position: 'fixed', top: '0', left: '0', width: '100%', height: '10px', background: '#f48fb1',
            color: '#333', fontWeight: 'bold', fontSize: '10px', textAlign: 'center', lineHeight: '10px',
            letterSpacing: '1px', zIndex: '9999', overflow: 'hidden', whiteSpace: 'nowrap'
          });
